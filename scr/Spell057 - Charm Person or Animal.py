...
def OnBeginSpellCast( spell ):
	print "Charm Person or Animal OnBeginSpellCast"
	print "spell.target_list=", spell.target_list
	print "spell.caster=", spell.caster, " caster.level= ", spell.caster_level
	game.particles( "sp-enchantment-conjure", spell.caster )

def	OnSpellEffect( spell ):
	print "Charm Person or Animal OnSpellEffect"
	...
	if not target_item.obj.is_friendly( spell.caster ):
		...
		if (target_item.obj.is_category_type( mc_type_animal ) == 1):
			...
			if not target_item.obj.saving_throw_spell( spell.dc, D20_Save_Will, D20STD_F_NONE, spell.caster, spell.id ):
				...
				target_item.obj.condition_add_with_args( 'sp-Charm Person or Animal', spell.id, spell.duration, target_item.obj.hit_dice_num )
				target_item.partsys_id = game.particles( 'sp-Charm Person or Animal', target_item.obj )
...
def OnBeginRound( spell ):
	print "Charm Person or Animal OnBeginRound"

def OnEndSpellCast( spell ):
	print "Charm Person or Animal OnEndSpellCast"