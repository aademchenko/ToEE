...
from py00439script_daemon import san_new_map, record_time_stamp, get_f, set_f, get_v, set_v, npc_set, npc_unset, npc_get, tsc
from py00334generic_spawner import hickory_branch_check
...
def check_repeatable_encounter( setup, encounter ):
	...
	#while (encounter.dc > (party_level+2)):
	countt = 0
	while countt < 5 and (encounter.dc > (party_level+2)  or ( party_level > 10 and encounter.dc < 5 and game.global_flags[500] == 1 and game.random_range(1,100) <= 87 )  ): # will reroll the encounter for higher levels (but still about 13% chance for lower level encounter, i.e. about 1/7 )
		get_repeatable_encounter_enemies(setup,encounter)
		countt += 1
	if countt == 5:
		return 0
...
def get_scrub_daytime( encounter ):
	...
	re_list.append( RE_entry( 6,    ( (14217, 1*m, 4*m, 1),  ),      1045 )     ) # hill giants
	...
	if game.global_flags[500] == 1:
		re_list.append( RE_entry( 9,    ( (14892, 1*m, 2*m, 0.5), (14891, 2*m, 4*m, 0.5) ,(14890, 2*m2, 3*m2, 0.5) ,(14889, 1*m2, 2*m2, 0.5), ),      1022 )     ) # Lizardman battlegroup
		re_list.append( RE_entry( 9,    ( (14888, 3, 5, 0.5), (14891, 0*m, 1*m, 0.5) ,(14696, 0*m2, 1*m2, 0.5) ,(14896, 0*m2, 1*m2, 0.5), (14506, 0*m2, 1*m2, 0.5), (14527, 0*m2, 1*m2, 0.5), (14525, 0*m2, 1*m2, 0.5), ),      1024 )     ) # Cult of the Siren + Random Thrall
		re_list.append( RE_entry( 7,    ( (14898, 2*m2, 4*m2, 0.5), (14897, 2*m2, 4*m2, 0.5) ,),      1022 )     ) # Leucrottas + Jackalweres
		re_list.append( RE_entry( 9,    ( (14248, 1, 1, 1), (14249, 3*m, 5*m), ) ,      1016 )     ) # Ogre chief and ogres
		re_list.append( RE_entry( 9,    ( (14248, 1, 1, 1), (14249, 3*m, 5*m), (14697, 3*m, 5*m), ) ,      1023 )     ) # Ogre chief and ogres vs. ettins (clash of the titans! :) )
		if game.party_alignment &  ALIGNMENT_EVIL != 0:
			re_list.append( RE_entry( 10,    ( (14896, 2, 4, 1), (14895, 3, 5), (14894, 2, 3, 0.5) ,),      1022 )     ) # Holy Rollers
...
def get_scrub_nighttime( encounter ):
	...
	if game.global_flags[500] == 1:
		re_list.append( RE_entry( 9,    ( (14892, 1*m2, 2*m2, 0.5), (14891, 2*m, 4*m, 0.5) ,(14890, 2*m2, 3*m2, 0.5) ,(14889, 1*m2, 2*m2, 0.5), ),      1022 )     ) # Lizardman battlegroup
		re_list.append( RE_entry( 7,    ( (14898, 2*m2, 4*m2, 0.5), (14897, 2*m2, 4*m2, 0.5) ,),      1022 )     ) # Leucrottas + Jackalweres
		re_list.append( RE_entry( 5,    ( (14542, 2*m, 4*m, 1),  ) ,      1017 )     ) # Invisible Stalkers
		re_list.append( RE_entry( 9,    ( (14248, 1, 1, 1), (14249, 3*m, 5*m), ) ,      1016 )     ) # Ogre chief and ogres
		re_list.append( RE_entry( 5,    ( (14510, 1*m, 3*m, 1), (14299, 1*m, 3*m) ) ,      1028 )     ) # Huge Fire elementals and fire snakes
		re_list.append( RE_entry( 12,    ( (14958, 1*m, 1*m, 1), (14893, 2*m, 4*m, 1), ) ,      1017 )     ) # Nightwalker and Greater Shadows
	aaa = game.random_range(0, len(re_list)-1 )
	encounter.enemies = re_list[aaa].get_enemies()
	encounter.dc = re_list[aaa].dc
	encounter.id = re_list[aaa].id		
...
def get_forest_daytime( encounter ):
	...
	if game.global_flags[500] == 1:
		re_list.append( RE_entry( 7,    ( (14898, 2*m2, 4*m2, 0.5), (14897, 2*m2, 4*m2, 0.5) ,),      1012 )     ) # Leucrottas + Jackalweres
		re_list.append( RE_entry( 9,    ( (14888, 3, 5, 0.5), (14891, 0*m, 1*m, 0.5) ,(14696, 0*m2, 1*m2, 0.5) ,(14896, 0*m2, 1*m2, 0.5), (14506, 0*m2, 1*m2, 0.5), (14527, 0*m2, 1*m2, 0.5), (14525, 0*m2, 1*m2, 0.5), ),      1013 )     ) # Cult of the Siren + Random Thrall
		re_list.append( RE_entry( 5,    ( (14542, 2*m, 4*m, 1),  ) ,      1014 )     ) # Invisible Stalkers
		re_list.append( RE_entry( 9,    ( (14248, 1, 1, 1), (14249, 3*m, 5*m), ) ,      1015 )     ) # Ogre chief and ogres
		if game.party_alignment &  ALIGNMENT_EVIL != 0:
			re_list.append( RE_entry( 10,    ( (14896, 2*m, 4*m, 1), (14895, 3*m, 5*m), (14894, 2*m, 3*m, 0.5) ,),      1016 )     ) # Holy Rollers
...
def get_forest_nighttime( encounter ):
	...
	# higher level encounters
	re_list.append( RE_entry( 5,    ( (14291, 2*m, 3*m, 1),  ) ,      1014 )     ) # Will o' wisps

	if game.global_flags[500] == 1:
		re_list.append( RE_entry( 7,    ( (14898, 2*m2, 4*m2, 0.5), (14897, 2*m2, 4*m2, 0.5) ,),      1015 )     ) # Leucrottas + Jackalweres
		re_list.append( RE_entry( 6,    ( (14674, 2, 3, 1),  (14280, 1, 2, 1), (14137, 2*m, 4*m), ) ,      1016 )     ) # mohrgs and groaning spirits and ghasts
		re_list.append( RE_entry( 9,    ( (14248, 1, 1, 1), (14249, 3*m, 5*m), ) ,      1017 )     ) # Ogre chief and ogres
		re_list.append( RE_entry( 5,    ( (14542, 2*m, 4*m, 1),  ) ,      1018 )     ) # Invisible Stalkers
		re_list.append( RE_entry( 11,    ( (14958, 1, 1, 1), (14893, 2*m, 4*m, 0.5), ) ,      1019 )     ) # Nightwalker and Greater Shadows
...
def get_swamp_daytime( encounter ):
	...
	if game.global_flags[500] == 1:
		re_list.append( RE_entry( 9,    ( (14892, 1*m, 2*m, 0.5), (14891, 2*m, 4*m, 0.5) ,(14890, 2*m2, 3*m2, 0.5) ,(14889, 1*m2, 2*m2, 0.5), ),      1023 )     ) # Lizardman battlegroup
		re_list.append( RE_entry( 9,    ( (14892, 1, 2, 0.5), (14891, 2, 4, 0.5) ,(14890, 2*m2, 3*m2, 0.5) ,(14889, 1*m2, 2*m2, 0.5), (14343, 1*m2, 2*m2), (14090, 1*m2, 2*m2) ),      1024 )     ) # Lizardman battlegroup + lizards + hydras
		re_list.append( RE_entry( 5,    ( (14343, 1*m, 2*m, 1),  ),      1025 )     ) # Hydras
		re_list.append( RE_entry( 9,    ( (14261, 1*m, 4*m, 1), ),      1026 )     ) # Vodyanoi
		re_list.append( RE_entry( 6,    ( (14279, 2, 3, 1), (14375, 2, 4),  ),      1027 )     ) # Seahags and watersnakes
...
def get_swamp_nighttime( encounter ):
	...
	re_list.append( RE_entry( 7,    ( (14280, 1*m, 1*m), ),      1005 )     ) # groaning spirit
	...
	if game.global_flags[500] == 1:
		re_list.append( RE_entry( 9,    ( (14892, 1*m2, 2*m2, 0.5), (14891, 2*m, 4*m, 0.5) ,(14890, 2*m2, 3*m2, 0.5) ,(14889, 1*m2, 2*m2, 0.5), ),      1008 )     ) # Lizardman battlegroup
		re_list.append( RE_entry( 9,    ( (14892, 1, 2, 0.5), (14891, 2, 4, 0.5) ,(14890, 2*m2, 3*m2, 0.5) ,(14889, 1*m2, 2*m2, 0.5), (14343, 1*m2, 2*m2), (14090, 1*m2, 2*m2) ),      1009 )     ) # Lizardman battlegroup + lizards + hydras
		re_list.append( RE_entry( 11,    ( (14958, 1, 1, 1), (14893, 2*m, 4*m, 1), ) ,      1010 )     ) # Nightwalker and Greater Shadows
		re_list.append( RE_entry( 5,    ( (14343, 1*m, 2*m, 1),  ),      1011 )     ) # Hydras
		re_list.append( RE_entry( 9,    ( (14261, 1*m, 4*m, 1), ),      1012 )     ) # Vodyanoi
		re_list.append( RE_entry( 6,    ( (14279, 1*m, 3*m, 1), (14375, 1*m, 3*m),  ),      1013 )     ) # Seahags and watersnakes
		re_list.append( RE_entry( 5,    ( (14824, 1*m, 3*m, 1), (14825, 1*m, 3*m, 1), ),      1014 )     ) # Ettin & Hill giant zombies
...
def get_riverside_daytime( encounter ):
	...
	if game.global_flags[500] == 1:
		re_list.append( RE_entry( 9,    ( (14892, 1*m, 2*m, 0.5), (14891, 2*m, 4*m, 0.5) ,(14890, 2*m2, 3*m2, 0.5) ,(14889, 1*m2, 2*m2, 0.5), ),      1008 )     ) # Lizardman battlegroup
		re_list.append( RE_entry( 9,    ( (14888, 3, 5, 0.5), (14891, 0*m, 1*m, 0.5) ,(14696, 0*m2, 1*m2, 0.5) ,(14896, 0*m2, 1*m2, 0.5), (14506, 0*m2, 1*m2, 0.5), (14527, 0*m2, 1*m2, 0.5), (14525, 0*m2, 1*m2, 0.5), ),      1009 )     ) # Cult of the Siren + Random Thrall
		re_list.append( RE_entry( 9,    ( (14892, 1*m, 2*m, 0.5), (14891, 2*m, 4*m, 0.5) ,(14890, 2*m2, 3*m2, 0.5) ,(14889, 1*m2, 2*m2, 0.5), (14279, 1*m, 3*m ), ),      1010 )     ) # Lizardman battlegroup + seahag
		re_list.append( RE_entry( 9,    ( (14261, 1*m, 4*m, 1), ),      1011 )     ) # Vodyanoi
		re_list.append( RE_entry( 7,    ( (14279, 1*m2, 3*m2, 1), (14375, 1*m, 3*m), (14240, 1*m, 3*m), ),      1012 )     ) # Seahags and watersnakes and kapoacinths
		if game.party_alignment &  ALIGNMENT_EVIL != 0:
			re_list.append( RE_entry( 10,    ( (14896, 2*m2, 4*m2, 1), (14895, 3*m2, 5*m2), (14894, 2*m2, 3*m2, 0.5) ,),      1013 )     ) # Holy Rollers
...
def get_riverside_nighttime( encounter ):
	...
	if game.global_flags[500] == 1:
		re_list.append( RE_entry( 9,    ( (14892, 1*m, 2*m, 0.5), (14891, 2*m, 4*m, 0.5) ,(14890, 2*m2, 3*m2, 0.5) ,(14889, 1*m2, 2*m2, 0.5), ),      1011 )     ) # Lizardman battlegroup
		re_list.append( RE_entry( 9,    ( (14892, 1*m, 2*m, 0.5), (14891, 2*m, 4*m, 0.5) ,(14890, 2*m2, 3*m2, 0.5) ,(14889, 1*m2, 2*m2, 0.5), (14279, 1*m, 3*m ), ),      1012 )     ) # Lizardman battlegroup + seahag
		re_list.append( RE_entry( 9,    ( (14261, 1*m, 4*m, 1), ),      1013 )     ) # Vodyanoi
		re_list.append( RE_entry( 6,    ( (14279, 1*m, 3*m, 1), (14375, 1*m, 3*m), (14240, 1*m, 3*m), ),      1014 )     ) # Seahags and watersnakes and kapoacinths
...
def can_sleep():
	...
	# wonnilon hideout
		...
		#else:
			return SLEEP_DANGEROUS
	elif (game.leader.map == 5065 and game.global_flags[840] == 1):
	# bandit hideout
			return SLEEP_SAFE
