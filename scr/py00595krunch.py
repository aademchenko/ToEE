...
def san_first_heartbeat( attachee, triggerer ):
	if (attachee.map == 5115):
		if (game.global_vars[985] == 1 and game.global_flags[565] == 0):
...
def san_dying( attachee, triggerer ):
	...
	game.global_vars[985] = 3
	game.global_flags[565] = 1
	return RUN_DEFAULT
		...
		if (not game.combat_is_active()):
			if (attachee != OBJ_HANDLE_NULL and critter_is_unconscious(attachee) != 1 and not attachee.d20_query(Q_Prone) and attachee.leader_get() == OBJ_HANDLE_NULL):
				if (game.global_vars[986] != 3):
					...
					if (game.global_vars[985] == 1 and not npc_get(attachee,2)):
						game.timevent_add( krunch_exit, ( attachee, triggerer ), 200 )
						npc_set(attachee,2)
					if ((game.global_flags[555] == 1 or game.global_flags[556] == 1) and game.global_flags[559] == 1):
...
def krunch_exit( attachee, triggerer ):
...
#	attachee.obj_set_int(obj_f_speed_walk, 1085353216)
	...
	game.timevent_add( krunch_off, ( attachee, triggerer ), 8000 )
	game.timevent_add( kos_all, ( attachee, triggerer ), 2000 )
	return RUN_DEFAULT


def krunch_off( attachee, triggerer ):
	attachee.object_flag_set(OF_OFF)
	npc_set(attachee,3)
	return RUN_DEFAULT
