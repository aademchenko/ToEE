...
def t_mode():
	try:
		ff_t_mode = open('test_mode.ini', 'r')
...
def kuf():
	# Kill unfriendlies
	for moshe in game.obj_list_vicinity(game.leader.location, OLC_NPC):
		if (moshe.reaction_get(game.party[0]) <= 0 or moshe.is_friendly(game.party[0]) == 0) and (moshe.leader_get() == ohn and moshe.object_flags_get() & OF_DONTDRAW == 0):
			# moshe.critter_kill_by_effect()
			damage_dice = dice_new( '50d50' )
			moshe.damage( game.party[0], 0, damage_dice )
...
def portraits_verify():
	...
	previous_portrait = -1
	found_error_flag = 0
	ff = open('portrait_checking_result.txt','w')
