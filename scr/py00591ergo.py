...
def san_first_heartbeat( attachee, triggerer ):
	if (attachee.map == 5095):
		if (game.global_vars[981] == 1 and game.global_vars[986] == 3 and game.global_flags[567] == 0):
		## turns on ergo outside cave
			attachee.object_flag_unset(OF_OFF)
	elif (attachee.map == 5115):
		if (game.global_vars[981] == 2 and game.global_flags[567] == 0):
...
def san_dying( attachee, triggerer ):
	...
	game.global_vars[981] = 3
	game.global_flags[567] = 1
	return RUN_DEFAULT
		...
		if (not game.combat_is_active()):
			if (attachee != OBJ_HANDLE_NULL and critter_is_unconscious(attachee) != 1 and not attachee.d20_query(Q_Prone) and attachee.leader_get() == OBJ_HANDLE_NULL):
				if (game.global_vars[986] != 3):
					...
					if (game.global_vars[981] == 1 and not npc_get(attachee,2)):
						game.timevent_add( ergo_exit, ( attachee, triggerer ), 200 )
						npc_set(attachee,2)
...
def ergo_exit( attachee, triggerer ):
	attachee.npc_flag_unset(ONF_WAYPOINTS_DAY)
	attachee.npc_flag_unset(ONF_WAYPOINTS_NIGHT)
	attachee.standpoint_set( STANDPOINT_NIGHT, 422 )
	attachee.standpoint_set( STANDPOINT_DAY, 422 )
#	attachee.standpoint_set( STANDPOINT_SCOUT, 422 )
#	attachee.obj_set_int(obj_f_speed_walk, 1085353216)
	attachee.runoff( location_from_axis( 588, 482 ) )
	game.timevent_add( ergo_off, ( attachee, triggerer ), 8000 )
	return RUN_DEFAULT


def ergo_off( attachee, triggerer ):
	attachee.object_flag_set(OF_OFF)
	npc_set(attachee,3)
	return RUN_DEFAULT
