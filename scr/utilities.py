	...
	for obj in game.leader.group_list():
		if ( (obj.is_unconscious() == 0 ) or (conscious_only == 0) ) and (obj.type == obj_t_pc or mode_select >= 1) and (obj.d20_query(Q_ExperienceExempt) == 0 or mode_select == 2 or obj.type == obj_t_pc):
			if closest_one == OBJ_HANDLE_NULL:
				if (obj.d20_query_has_spell_condition(sp_Otilukes_Resilient_Sphere) == 0 or exclude_warded == 0) and (obj.d20_query(Q_Critter_Is_Charmed) == 0 or exclude_charmed == 0  ) and ( ( not obj.name in [14370, 14604, 14621, 14629] ) or exclude_spiritual_weapon == 0 ):
					closest_one = obj
...
def willing_and_capable( attachee ):
